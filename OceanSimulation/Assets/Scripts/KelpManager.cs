...
public class KelpManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        chanceOfKelpToSpawn = UniversalStats.Instance.chanceOfKelpToSpawn;
        totalNumOfKelp = UniversalStats.Instance.totalNumOfKelp;
    ...
    private void Update()
    {
        if (deactivatedKelp.Count > 0 && Random.Range(0, 100) < chanceOfKelpToSpawn * Time.deltaTime)
        {
            int numOfKelpToSpawn = Mathf.Clamp( Mathf.RoundToInt(Random.Range(chanceOfKelpToSpawn * 0.005f,
                chanceOfKelpToSpawn * 0.001f) * (totalNumOfKelp - deactivatedKelp.Count)), 1, 20);
    ...
    public void spawnKelp(float _x, float _z)
    {
        ...
        UniversalStats.Instance.totalNumOfKelp++;
    }
}
