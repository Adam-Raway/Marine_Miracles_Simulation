...
public class AnimalStats : MonoBehaviour
{
    ...
    void Start()
    {
        hunger = UnityEngine.Random.Range(0, maxHunger * 0.75f);
        age = UnityEngine.Random.Range(0, ageOfDeath - 1);
        if (age >= ageOfMaturity) reproductiveUrge = UnityEngine.Random.Range(0, 10);
    ...
    public void die()
    {
        if (gameObject.layer == 10) UniversalManager.Instance.totalPopulationOfCrabs--;
        else if (gameObject.layer == 11) UniversalManager.Instance.totalPopulationOfSalmon--;
        else if (gameObject.layer == 12) UniversalManager.Instance.totalPopulationOfTurtles--;
